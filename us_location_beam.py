...
class FormatStateFn(beam.DoFn):
    def process(self, element):
        ...
        latitude = cusa_record.get('latitude')
        longitude = cusa_record.get('longitude')
        fips = cusa_record.get('fips')
        admin2 = cusa_record.get('admin2')
        combined_key = cusa_record.get('combined_key')
        ...
        if province_state is not None:
            ...
            if len(split_state) > 1:
                ...
                print('state', state)
                return [{'id':element_id, 'city_county':city_county, 'province_state':province_state, 'country_region':country_region, 'latitude':latitude, 'longitude':longitude, 'fips':fips,
                         'admin2':admin2, 'combined_key':combined_key}]
        return [{'id':element_id, 'city_county':city_county, 'province_state':province_state, 'country_region':country_region, 'latitude':latitude, 'longitude':longitude, 'fips':fips,
                         'admin2':admin2, 'combined_key':combined_key}]
...
def run():
    ...
    sql = 'SELECT id, province_state, country_region, latitude, longitude, fips, admin2, combined_key FROM covid19_jhu_csse_modeled.us_location limit 20'
    ...
    table_id = 'us_location_Beam'
    schema_id = 'id:INTEGER, city_county:STRING, province_state:STRING, country_region:STRING, latitude:FLOAT, longitude:FLOAT, fips:STRING, admin2:STRING, combined_key:STRING'
